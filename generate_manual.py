...
def generate_markdown(config: dict, sprites: list) -> str:
    ...
    def ul(t): lines.append(f"- {t}")
    def ol(t): lines.append(f"1. {t}")
    def code(t): lines.append(f"```{t}```\n")
    ...
    p(f"**Написано с помощью ИИ-агентов**\n")
...
""")
    ...
    # ---------------- 5. Бонусы ----------------
    h2("5. Бонусы")
    p("После съедания обычного яблока с вероятностью "
    ...
    p("При разблокировке достижения в центре экрана появляется всплывающее уведомление.")
    lines.append("")
    ...
    ul("**Панель лидеров** — справа от поля, показывает топ-10 игроков.")
    ul("**Информационная строка** — под полем: счёт, рекорд, активные эффекты, статус.")
    ul("**Кнопки** — «🎨 Тема (T)» и «🏆 Ачивки (A)».")
    ...
    table(
        ...
        [
            ...
            ["slow_step_penalty", "float", config.get("slow_step_penalty", 0.04), "Добавка к шагу при замедлении (сек)"],
    ...
    lines.append("---")
    p("**Приятной игры! 🐍**  ")
    p(f"Руководство создано {datetime.now().strftime('%d.%m.%Y')} с помощью ИИ-агентов.  ")
